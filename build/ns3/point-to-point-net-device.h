...
public:
	...
	/*
	 * Check whether this packet is forwarding packet or backtracking packet.
	 *
	 * Returns true if it is a forwarding packet, otherwise returns false
	 * */
	bool IsForwarding_FatTree(NodeId nodeId, IdTag dstId, uint32_t iif);
    /**
     * Check whether this link is failure
     * Return true if this link is not failure, otherwise return false.
     * */
	...
	/**
	 * Check if there exists an path down to the destination server.
	 *
	 * Return true if exists, return false if not.
	 */
	bool IsDestReachable(NodeId switchId, IdTag dstId);
	/**
	 * Updating turning switch information according to the re-routing map
	 *
	 * All the packets, arriving after re-routing algorithm executed,
	 * will change their turning switch tag in the packet header.
	 */
